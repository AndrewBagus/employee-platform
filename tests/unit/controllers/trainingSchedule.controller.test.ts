...
describe("TrainingScheduleController", () => {
  test("GET /:id returns 404 when not found", async () => {
    ...
    container.rebind(TYPES.TrainingScheduleServiceInterface).toConstantValue(mockService);
    const { default: ctrl } = await import(
      "../../../src/controllers/trainingSchedule.controller.ts?t=" + Date.now()
    );
