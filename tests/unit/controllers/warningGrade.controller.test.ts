...
describe("WarningGradeController", () => {
  test("GET /:id returns 404 when not found", async () => {
    ...
    container.rebind(TYPES.WarningGradeServiceInterface).toConstantValue(mockService);
    const { default: ctrl } = await import(
      "../../../src/controllers/warningGrade.controller.ts?t=" + Date.now()
    );
