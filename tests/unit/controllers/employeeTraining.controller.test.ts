...
describe("EmployeeTrainingController", () => {
  test("GET /:id returns 404 when not found", async () => {
    ...
    container.rebind(TYPES.EmployeeTrainingServiceInterface).toConstantValue(mockService);
    const { default: ctrl } = await import(
      "../../../src/controllers/employeeTraining.controller.ts?t=" + Date.now()
    );
