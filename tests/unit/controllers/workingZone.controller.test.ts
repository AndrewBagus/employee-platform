...
describe("WorkingZoneController", () => {
  test("GET /:id returns 404 when not found", async () => {
    ...
    container.rebind(TYPES.WorkingZoneServiceInterface).toConstantValue(mockService);
    const { default: ctrl } = await import(
      "../../../src/controllers/workingZone.controller.ts?t=" + Date.now()
    );
