...
describe("EmployeeWarningController", () => {
  test("GET /:id returns 404 when not found", async () => {
    ...
    container.rebind(TYPES.EmployeeWarningServiceInterface).toConstantValue(mockService);
    const { default: ctrl } = await import(
      "../../../src/controllers/employeeWarning.controller.ts?t=" + Date.now()
    );
