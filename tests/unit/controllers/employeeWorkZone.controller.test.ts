...
describe("EmployeeWorkZoneController", () => {
  test("GET /:id returns 404 when not found", async () => {
    ...
    container.rebind(TYPES.EmployeeWorkZoneServiceInterface).toConstantValue(mockService);
    const { default: ctrl } = await import(
      "../../../src/controllers/employeeWorkZone.controller.ts?t=" + Date.now()
    );
